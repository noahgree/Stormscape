...
func add_xp(amount: int) -> bool:
	...
	while allowed_lvl < MAX_LEVEL:
		var xp_needed: int = xp_needed_for_lvl(stats, allowed_lvl + 1)
	...
	if DebugFlags.weapon_xp_updates:
		var xp_needed_now: int = xp_needed_for_lvl(stats, allowed_lvl + 1)
...
func migrate_from_rarity_upgrade(original_wpn: WeaponStats, source_entity: Entity,
	...
	uid = original_wpn.uid
	WeaponModsManager.copy_mods_between_weapons(original_wpn, stats, source_entity, remove_from_orig)
...
func print_total_needed(for_level: int) -> void:
	...
	for lvl: int in range(1, for_level + 1):
		var xp: int = xp_needed_for_lvl(stats, lvl)
		total += xp
